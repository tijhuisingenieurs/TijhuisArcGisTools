...
# monstervakken_doelbestand = './testdata/output/2_d2_output_monstervakken.shp'
# doelbestand_boorpunten = './testdata/output/2_d2_output_boorpunten.shp'
...
for p in boorpunten_col.filter():
    ...
    mv_value = None
    mv_value_hernummer = None
    for field in fields:
        if field == monstervak_veld:
            mv_value = p['properties'][field]
        if field == 'hernummer':
            mv_value_hernummer = p['properties'][field]
    ...
    point.Y = p['geometry']['coordinates'][1]
    row.Shape = point
    row.setValue('mv_code', mv_value)
    row.setValue('hernummer', mv_value_hernummer)
